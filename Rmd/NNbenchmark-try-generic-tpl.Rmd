...
str(ZZ)
resfit <- do.call("NNtrain.AMORE", list(ZZ$x, ZZ$y, neur, method[1],  hyperParams.AMORE))
str(resfit)
...
train_and_predict_1mth_1data <- function(dset, method, trainFUN, hyperparamFUN, predictFUN, echofile=NULL)
{
    ds     <- NNdatasets[[dset]]$ds
    Z      <- NNdatasets[[dset]]$Z
    neur   <- NNdatasets[[dset]]$neur
    ...
    if(!is.null(echofile))
        save(respred, resfit, file=echofile)
    c(RMSE = NNbenchmark::funRMSE(respred, ZZ$y0), MAE= NNbenchmark::funMAE(respred, ZZ$y0))
}

resallmethod <- sapply(1:length(method), function(i)
       train_and_predict_1mth_1data(1, method[i], "NNtrain.AMORE", hyperParams.AMORE, "NNpredict.AMORE")
)
colnames(resallmethod) <- method

library(parallel)

train_and_predict_1data <- function(dset, method, trainFUN, hyperparamFUN, predictFUN, echofile=NULL)
{
    resallmethod <- sapply(1:length(method), function(i)
       train_and_predict_1mth_1data(dset, method[i], trainFUN, hyperparamFUN, predictFUN, echofile))
    colnames(resallmethod) <- method
    resallmethod
}
