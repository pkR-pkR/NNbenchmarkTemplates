...
Should be  part of the `NNbenchmark` package?!
    ...
    for(i in 1:nrep)
    {
        ...
        }else
        {
          ...
          if(echo && echoreport > 1)
          {
            ...
            print(str(do.call(predictFUN, list(tempfit, ZZ$x, ZZ$xy))))
          }
          Ypred[[i]] <- tryCatch(
...
## CaDENCE Train/predict Function
```{r}
library(CaDENCE)
...
## deepnet Train/predict Function
```{r}
library(deepnet)
...
## MachineShop Train/predict Functions
```{r}
library(MachineShop)
...
## minpack.lm train/predict functions
```{r}
library(minpack.lm)
...
## monmlp train/predict functions
```{r}
library(monmlp)
...
## neural train/predict functions
```{r}
library(neural)
neural.method <- "none"
...
## neuralnet train/predict functions
```{r}
library(neuralnet)
neuralnet.method <- c("slr", "sag", "rprop-", "rprop+", "backprop")
hyperParams.neuralnet <- function(...) {
    return (list(iter=300, silent=TRUE, scale=TRUE))
}
...
res <- train_and_predict_1data(1, neuralnet.method, "NNtrain.neuralnet", "hyperParams.neuralnet", "NNpredict.neuralnet", 
                               ...
                               echo=TRUE, doplot=FALSE, echoreport=2,
                               pkgname="neuralnet", pkgfun="neuralnet", rdafile=TRUE, odir=odir)
```

## nlsr train/predict functions
```{r}
library(nlsr)
nlsr.method <- "none"
...
## radiant.model Train/predict Functions
```{r}
...
## rminer Train/predict Functions
```{r}
library(rminer)
rminer.method <- "none"
...
methodlist <- list(AMORE.method, automl.method, brnn.method, CaDENCE.method, 
                   deepnet.method, elmNNRcpp.method, ELMR.method, h2o.method, 
                   keras.method, MachineShop.method, minpack.lm.method, monmlp.method,
                   neural.method, nlsr.method, nnet.method, radiant.model.method,
                   rminer.method)
...
preparelist <- list(AMORE.prepareZZ, automl.prepareZZ, brnn.prepareZZ, CaDENCE.prepareZZ,
                    deepnet.prepareZZ, elmNNRcpp.prepareZZ, ELMR.prepareZZ, h2o.prepareZZ, 
                    keras.prepareZZ, MachineShop.prepareZZ, minpack.lm.prepareZZ, monmlp.prepareZZ, 
                    neural.prepareZZ, nlsr.prepareZZ, nnet.prepareZZ, radiant.model.prepareZZ,
                    rminer.prepareZZ)
...
resall <- train_and_predict_1data(dset=1, method=methodlist, train=trainvect, hyper=hypervect,
                                  ...
                                  start=startvect, prepare=preparelist, 
