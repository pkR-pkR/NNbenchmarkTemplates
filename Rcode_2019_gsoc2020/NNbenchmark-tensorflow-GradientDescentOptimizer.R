...
## =================================================================
## 2019-08-23 NNbenchmark TEMPLATE FOR tensorflow_1.14.0.9000 + Adam
...
for (dset in names(NNdatasets)) {
  ...
  params  <- "maxiter = 5000"
  comment <- ""
  descr   <- paste(dset,  "tensorflow::GradientDescentOptimizer", sep = "_")
  ...
  for(i in 1:nruns){
    ...
    generator <- tf$compat$v1$train$GradientDescentOptimizer(learning_rate = 0.001)
