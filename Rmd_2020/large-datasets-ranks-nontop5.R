...
lf <- csv::as.csv(paste0(odir, "/bWoodN1-results-nontop5.csv"))
head(lf)
...
summary.glf <- cbind.data.frame(pfa=unique(glf$pfa),                       
...
npfa <- function(x)
  switch (x,
    "CaDENCE::cadence.fit_optim"=12,
    "h2o::deeplearning_first-order"=24,
    "monmlp::monmlp.fit_BFGS"=34,
    "monmlp::monmlp.fit_Nelder-Mead"=35,
    "radiant.model::nn_none"=44,
    "traineR::train.nnet_none"=55,
    NA
  )
summary.glf$npfa <- sapply(summary.glf$pfa, npfa)
...
scoredfr0 <- as.data.frame(summary.glf[, 
        c("pfa","time.mean","RMSE.min","RMSE.med",
          "RMSE.d51","MAE.med","WAE.med","npfa")])

scoredfr <- scoredfr0[order(scoredfr0$RMSE.min),]
write.csv(scoredfr0, paste0(odir, "/bWoodN1-results-summary-nontop5.csv"),
          row.names = FALSE)
