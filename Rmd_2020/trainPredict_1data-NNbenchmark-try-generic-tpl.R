...
if(FALSE)
res <- train_and_predict_1data(1, softmaxreg.method, "NNtrain.softmaxreg", "hyperParams.softmaxreg", "NNpredict.softmaxreg", 
...
resall <- lapply(1:3, function(i)
  train_and_predict_1data(dset=i, method=methodlist[[1]], train=trainvect[1], hyper=hypervect[1],
                          pred=predvect[1], summary=NNsummary, close=closevect[1], 
                          start=startvect[1], prepare=preparelist[[1]], nrep=2, echo=FALSE,
                          doplot=FALSE, pkgname=pkgfunmat[1,"pkg"], pkgfun=pkgfunmat[1,"fun"],
                          rdafile=TRUE, odir=odir))
if(is.list(resall))
{
