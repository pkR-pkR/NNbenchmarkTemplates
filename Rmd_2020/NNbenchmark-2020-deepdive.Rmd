...
hyperParams.deepdive <- function(optim_method, ...) {
    ...
    if (optim_method == "gradientDescent")  {maxiter <- maxit1storderC; eta = 0.01} 
    if (optim_method == "momentum")         {maxiter <- maxit1storderC; eta = 0.8}
    if (optim_method == "rmsProp")          {maxiter <- maxit1storderB; eta = 0.8}
    if (optim_method == "adam")             {maxiter <- maxit1storderB; eta = 0.8}
    
    params <- paste0("method=", optim_method, "_iter=", maxiter)
  
    out <- list(iter = maxiter, method = optim_method, modelType = "regress", eta = eta, print = 1000, params)
    return (out)
}
