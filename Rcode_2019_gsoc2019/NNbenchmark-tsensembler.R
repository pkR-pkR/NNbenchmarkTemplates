...
## =====================================================
## 2019-08-24 NNbenchmark TEMPLATE FOR tsensembler_0.0.5
...
for (dset in names(NNdatasets)) {
  ...
  params  <- "maxiter = 150"
  comment <- "uses nnet"
  descr   <- paste(dset,  "tsensembler::tsensembler", sep = "_")
  ...
  for(i in 1:nruns){
    ...
    NNreg  <- tsensembler::bm_ffnn(fmla, Zxy, 
                                   list(size = neur, 
                                        maxit = maxiter, decay = 0))
    y_pred     <- tryCatch(
