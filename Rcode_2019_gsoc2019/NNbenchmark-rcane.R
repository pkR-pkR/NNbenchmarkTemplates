...
## ====================================================
## 2019-08-22 NNbenchmark TEMPLATE FOR rcane_1.0
...
for (dset in 1) {  
  ...
  params  <- "maxiter = 1000"
  comment <- "linear only"
  descr   <- paste(dset,  "rcane::rlm", sep = "_")
  method <- "sgd"
  ...
  #  NNreg <- rcane::rlm(fmla, Zxy, method = "sgd", alpha = 0.1, max.iter = 1000)
    
    NNreg      <- tryCatch(
      rcane::rlm(fmla, Z, method = method, alpha = 0.001, max.iter = maxiter),
