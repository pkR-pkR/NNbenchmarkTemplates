...
## ====================================================
## 2019-08-22 NNbenchmark TEMPLATE FOR neural_1.4.2.2
...
for (dset in names(NNdatasets)) {
  ...
  descr   <- paste(dset,  "neural::mlptrain", sep = "_")
  ...
  for(i in 1:nruns){
    ...
    NNreg      <- tryCatch(
      neural::mlptrain(x, neurons = neur, y, 
                       it = maxiter, alfa = 0.9, visual = F),
      error = function(y) {lm(y ~ 0, data = Zxy)}
    )
    y_pred     <- tryCatch(
      ym0 + ysd0*mlp(x, NNreg$weight, NNreg$dist, NNreg$neurons, NNreg$actfns),
